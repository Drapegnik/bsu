...
SELECT deptaddress FROM dept WHERE deptname = 'Sales';

--2.	Выдать информацию обо всех работниках, родившихся не ранее 1 января 1980 года.
SELECT * FROM emp WHERE birthdate >= to_date('01-01-1980', 'dd-mm-yyyy');

--3.	Найти минимальный оклад, предусмотренный для водителя  (Driver).
SELECT minsalary FROM job WHERE jobname = 'Driver';
...
SELECT COUNT(DISTINCT empno) FROM career
WHERE (startdate > to_date('31-05-2016','dd-mm-yyyy') and (startdate < enddate OR enddate is null));
...
SELECT DISTINCT career.jobno FROM career
...
WHERE (
	deptid = 'B02' AND 
	startdate >= to_date('01.01.2013','dd.mm.yyyy') AND 
	enddate <= to_date('31.12.2016','dd.mm.yyyy')
);
...
where (
  ...
  WHERE (
  	career.deptid = dept.deptid AND
    career.startdate >= to_date('01.01.2014','dd-mm-yyyy') AND 
    career.enddate <= to_date('31.12.2014','dd-mm-yyyy')
  )
) = 0;
...
WHERE (
	...
	WHERE bonus.empno = emp.empno AND bonus.year BETWEEN 2013 AND 2014
) = 0;
...
WHERE (
	SELECT COUNT(DISTINCT deptid) FROM career
...
INNER JOIN career ON career.empno = emp.empno 
WHERE (
	SELECT COUNT(DISTINCT jobno) FROM career
	WHERE career.empno = emp.empno
) >= 2;
